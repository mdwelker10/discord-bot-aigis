...
exports.checkToken = async () => {
  try {
    let expiry = config.get('MANGADEX_EXPIRE_TIME');
    let current = Math.floor(Date.now() / 1000) + 1000; // 1000 seconds before token expiry
    //token expired or about to expire
    if (current > expiry) {
      let authOptions = {};
      if (current > config.get('MANGADEX_REFRESH_EXPIRE_TIME')) {
        ...
        authOptions = {
          ...
          data: {
            grant_type: 'password',
            username: config.get('MANGADEX_USERNAME'),
            password: config.get('MANGADEX_PASSWORD'),
            client_id: config.get('MANGADEX_CLIENT_ID'),
            client_secret: config.get('MANGADEX_CLIENT_SECRET')
          }
        };
        let response = await axios(authOptions);
        config.set('MANGADEX_TOKEN', response.data.access_token);
        config.set('MANGADEX_REFRESH_TOKEN', response.data.refresh);
        config.set('MANGADEX_EXPIRE_TIME', Math.floor(Date.now() / 1000) + response.data.expires_in); //expires in 15 mins
        config.set('MANGADEX_REFRESH_EXPIRE_TIME', Math.floor(Date.now() / 1000) + response.data.refresh_expires_in); //expires in 90 days
      } else {
        //use refresh token
        authOptions = {
          method: 'post',
          ...
          data: {
            ...
            refresh_token: config.get('MANGADEX_REFRESH_TOKEN'),
            client_id: config.get('MANGADEX_CLIENT_ID'),
            client_secret: config.get('MANGADEX_CLIENT_SECRET')
          }
        };
    ...
    return config.get('MANGADEX_TOKEN');
  } catch (err) {
