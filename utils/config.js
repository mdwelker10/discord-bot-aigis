...
class Config {
  constructor() {
    ...
    this.defaults = {
      // App constants
      EMBED_COLOR: 0xFFC54A,
      TEMP_MAX_LENGTH: 50, // Max number of files in temp directory
      DAILY_TOKEN_AMOUNT: 100,
      DB_NAME: 'aigis',
      TEMP_PATH: path.join(__dirname, 'temp'),
      USER_AGENT: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:133.0) Gecko/20100101 Chrome/110 Firefox/133.0',
      MIN_BET: 5,
      MAX_BET: 1_000_000_000, // 1 billion
      DOWNLOADS_MAX_SIZE: 5 * 1024 * 1024 * 1024, // 5 GiB
      SOTD_MIN_PLAYLIST_LENGTH: 35,

      // Aigis images
      DEFAULT_MANGA_IMAGE: 'https://i.imgur.com/usdIJxN.png',
      AIGIS_BUSTUP_IMAGE: 'https://i.imgur.com/rqiiY75.png',
      AIGIS_YUKATA_IMAGE: 'https://i.imgur.com/6kKYY7X.png',
      AIGIS_DANCING_IMAGE: 'https://i.imgur.com/ozS5bjO.png',
      AIGIS_COOLDOWN_IMAGE: 'https://i.imgur.com/j0ilNtL.png',
    ...
    this.variables = {
      SPOTIFY_TOKEN: "",
      SPOTIFY_EXPIRE_TIME: 0,
      MANGADEX_TOKEN: "",
      MANGADEX_EXPIRE_TIME: 0,
      MANGADEX_REFRESH_TOKEN: "",
      MANGADEX_REFRESH_EXPIRE_TIME: ""
    }
  }

  /**
   * Get a configuration value by key
   * Priority: Environment variable > Default value > undefined
  ...
  /** 
   * Set a configuration value by key. Must be a variable value
   * @param {string} key - The configuration key to set
   * @param {*} value - The value to set
  */
  set(key, value) {
    if (!this.variables.hasOwnProperty(key)) {
      console.error(`Attempted to set non-variable configuration key "${key}"`);
      throw new AigisError(`Cannot set configuration key "${key}" because it is not a variable configuration.`);
    }
    this.variables[key] = value;
  }

  /**
   * Get all Mangadx ISO639-1 language codes
  ...
  /**
   * Get all Mangadx tags
   * @returns {Object} Tags mapping
   */
  getMangadxTags() {
    return {
...
// Export the instance and backward compatibility constants
